...
generate_options() {
  local IFS=' '
  eval printf 'with\ %s,' "n_bits={$n_bits}\ activation_observer={$activation_observer}\ weight_observer={$weight_observer}\ weight_range={$weight_range}\ use_dataset={$use_dataset}"
}
